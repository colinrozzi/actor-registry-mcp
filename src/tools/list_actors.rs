...
pub fn register_list_actors_tool(builder: ServerBuilder, registry: Registry) -> ServerBuilder {
    builder.with_tool(
        ...
        move |args| {
            ...
            debug!(
                "Listing actors with filter '{:?}', format '{}', detailed: {}",
                ...
                Ok(mut actors) => {
                    ...
                    if let Some(filter_str) = filter {
                        ...
                        // Check if it's a status filter
                            ...
                            actors.retain(|actor| match status {
                                "built" => actor.build_info.build_status == BuildStatus::Success,
                                "notbuilt" => {
                                    actor.build_info.build_status == BuildStatus::NotBuilt
                                }
                            ...
                            .map(|actor| {
                                ...
                                if detailed {
                                    if let Some(ref cargo) = actor.cargo_config {
                                        ...
                                        json["dependencies_count"] =
                                            json!(cargo.dependencies.len());
                                    }
                                }
