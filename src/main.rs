...
use tools::{
    create_actor::register_create_actor_tool,
    list_actors::register_list_actors_tool,
    build_actor::register_build_actor_tool,
    get_actor_info::register_get_actor_info_tool,
    get_actor_path::register_get_actor_path_tool,
};

#[tokio::main]
async fn main() -> Result<()> {
    // Initialize logging
    let log_file = "actor-registry-mcp.log";
    ...
    let registry_path = match env::var("THEATER_ACTORS_PATH") {
        ...
        Err(_) => {
            let default_path = PathBuf::from("/Users/colinrozzi/work/actors");
            info!("THEATER_ACTORS_PATH not set, using default path: {:?}", default_path);
            default_path
        }
    };
    ...
    let mut server_builder = ServerBuilder::new("theater-actor-registry", "0.1.0")
